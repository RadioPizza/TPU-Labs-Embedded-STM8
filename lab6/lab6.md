...
void tim1_init_pwm(void)
{
    ...
    TIM1_PSCRH = 0x07; // Старший байт делителя (PSCR = 1999)
    TIM1_PSCRL = 0xCF; // Младший байт делителя
    // Частота таймера = 2 МГц / (1999 + 1) = 1 кГц (1 такт = 1 мс)

    // Регистры автоперезагрузки (Auto-reload register)
    TIM1_ARRH = 0x00; // Старший байт ARR (ARR = 9)
    TIM1_ARRL = 0x09; // Младший байт ARR
    // Период ШИМ = (ARR + 1) * 1 мс = 10 мс

    // Регистры сравнения канала 1 (Capture/Compare register)
    TIM1_CCR1H = 0x00; // Старший байт CCR1 (CCR1 = 5)
    TIM1_CCR1L = 0x05; // Младший байт CCR1
    // Длительность импульса = CCR1 * 1 мс = 5 мс (скважность 2)
}
